    ...
    int    ThemeIndex) : IModel
{
    ...
    private static Style FocusedBorder => Style.Default.Border(Borders.Rounded);
    ...
    private Container Section(string label, IWidget body) => new Container(Axis.Vertical, [
            new Container(Axis.Vertical, height: SizeConstraint.Fixed(1), children: [
                new TextBlock("  " + label,
                    style: Secondary)
            ]),
    ...
    private static TextBlock Swatch(string label, IColor bg, IColor fg) =>
        new(label, style: Style.Default.Background(bg).Foreground(fg));

    private static Container ColoredBox(string label, IColor bg, SizeConstraint width,
